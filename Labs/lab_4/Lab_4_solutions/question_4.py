...
def get_problem_size_and_names():
    while True:
        try:
            problem_size = int(input('Enter a strictly positive number for the number of couples: '))
            if problem_size <= 0:
                raise ValueError
            break
        except ValueError:
...
def get_names(problem_size, gender, default_name):
    ...
    while True:
        try:
            individuals = print('Enter {} names for the {}, all on one line '
                                ' and separated by spaces,'.format(problem_size, gender))
            individuals = input('or just press Enter for the default "names" {1:}_1, ..., {1:}_{0:}: '
                                .format(problem_size, default_name)).split()
            ...
            print('\nYou did not give me {} distinct names, try again.'.format(problem_size))
...
def get_preferences_from_user(problem_size, gender_1, gender_2, gender_1_is_male):
    ...
    for individual in gender_1:
        while True:
            try:
                preferences = input('List preferences for {}, in decreasing order: '.

...
class LinkedList:
    ...
    def __init__(self, L = None, compare = lambda x, y: x <= y):
    ...
    def duplicate(self):
        ...
        LL = LinkedList(compare = self.compare)
        LL.head = node_copy
        node = node.next_node
        while node:
    ...
    def is_sorted(self):
        ...
        while node and node.next_node:
            if not self.compare(node.value, node.next_node.value):
                return False
            node = node.next_node
        return True
    ...
    def insert_sorted_value(self, value):
        ...
        if not self.head:
            self.head = new_node
            return
        if self.compare(value, self.head.value):
            new_node.next_node = self.head
            self.head = new_node
            return
        node = self.head
        while node.next_node and self.compare(node.next_node.value, value):
            node = node.next_node
        new_node.next_node = node.next_node
        node.next_node = new_node

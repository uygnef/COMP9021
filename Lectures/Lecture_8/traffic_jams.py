...
CIRCUIT_MIDDLE_RADIUS = ((CIRCUIT_OUTER_DIAMETER + CIRCUIT_INNER_DIAMETER)
                                                                       // 4)
...
class ControlBoard(Frame):
    def __init__(self, traffic):
        ...
        self.nb_of_cars_scale = Scale(self, length = 200, orient = HORIZONTAL, label ='́Number of cars:',
                                      from_ = MIN_NB_OF_CARS, to = MAX_NB_OF_CARS, tickinterval = 10,
                                      command = self.set_nb_of_cars)
        ...
        Scale(self, length = 150, orient = HORIZONTAL, label ='́Acceleration:',
              ...
              command = self.set_acceleration).pack(side = LEFT, padx = 50)
...
class Circuit(Frame):
    def __init__(self):
        Frame.__init__(self, bd = 10, padx = 20, pady = 20)
        self.road = Canvas(self, width = CIRCUIT_OUTER_DIAMETER + 2 * OFFSET,
                           height = CIRCUIT_OUTER_DIAMETER + 2 * OFFSET)
        self.road.create_oval(OFFSET, OFFSET, CIRCUIT_OUTER_DIAMETER + OFFSET,
                              CIRCUIT_OUTER_DIAMETER + OFFSET, fill = ROAD_COLOUR)
        self.road.create_oval(ROAD_WIDTH + OFFSET, ROAD_WIDTH + OFFSET,
                              CIRCUIT_INNER_DIAMETER + ROAD_WIDTH + OFFSET,
...
class Traffic:
    ...
    def generate_and_draw_cars(self):
        ...
        positions.sort()
        self.cars = Car(positions[0], speed = randrange(MIN_SPEED, MAX_SPEED + 1),
                        colour = self.random_color())
        current_car = self.cars
        for i in range(1, self.nb_of_cars):
            car = Car(positions[i], speed = randrange(MIN_SPEED, MAX_SPEED + 1),
                      colour = self.random_color())
    ...
    def draw_cars(self):
        for _ in range(self.nb_of_cars):
            self.road.delete(self.cars.drawn_car)
            x = (CIRCUIT_CENTER + cos(self.cars.position * 2 * pi / CIRCUIT_LENGTH) *
                                                                                   CIRCUIT_MIDDLE_RADIUS)
            y = (CIRCUIT_CENTER + sin(self.cars.position * 2 * pi / CIRCUIT_LENGTH) *
                                                                                   CIRCUIT_MIDDLE_RADIUS)
    ...
    def simulate(self):
        for _ in range(self.nb_of_cars):
            distance = ((self.cars.car_at_the_front.position - self.cars.position)
                                                                      % CIRCUIT_LENGTH - SAFETY_DISTANCE)
            ...
            self.cars.position = (self.cars.position + self.cars.speed *
                                                      FACTOR_FOR_DISTANCE_PER_ITERATION) % CIRCUIT_LENGTH
